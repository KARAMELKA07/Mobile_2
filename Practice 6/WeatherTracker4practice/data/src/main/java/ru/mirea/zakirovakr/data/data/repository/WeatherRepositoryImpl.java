        ...
        new Thread(() -> {
            try {
                ...
                if (!geoResp.isSuccessful() || geoResp.body() == null
                        || geoResp.body().results == null
                        || geoResp.body().results.isEmpty()) {
                ...
                String current = "temperature_2m,weather_code,relative_humidity_2m," +
                ...
                // Краткие поля для списка
                String humidity  = "Влажность: " + c.relativeHumidity2m + "%";
                String wind      = "Ветер: " + Math.round(c.windSpeed10m) + " м/с";

                // Детальные поля для экрана подробнее
                String feelsLike = "Ощущается как: " + Math.round(c.apparentTemperature) + "°C";
                String pressure  = "Давление: " + Math.round(c.pressureMsl) + " гПа";
                String clouds    = "Облачность: " + c.cloudCover + "%";

                // Локальное имя иконки (для списка)
                String iconName = mapConditionToIconName(condition);

                // Код иконки OpenWeather и URL (для детального экрана)
                String owCode  = WeatherCodeMapper.toOwIconCode(c.weatherCode);
                String iconUrl = WeatherCodeMapper.owIconUrl(owCode);
